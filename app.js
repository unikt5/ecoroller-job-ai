...
async function scanJobs() {
  ...
  state.jobs = dedupeJobs(batches.flat()).map(job => ({
    ...job,
...
async function searchAllSources(term, location) {
  const sources = [
    () => searchWorldCareersHub(term, location),
...
async function searchWorldCareersHub(term, location) {
  const url = new URL("https://worldcareershub.com/api/v1/jobs/search");
  url.searchParams.set("q", term);
  url.searchParams.set("location", location);

  const response = await fetch(url);
  if (!response.ok) throw new Error(`World Careers Hub error ${response.status}`);
  ...
  return jobs.map(job => normalizeJob({
    ...
    summary: job.description || job.summary || job.category || "",
    url: job.url,
    posted: job.posted || job.date,
    source: "World Careers Hub"
  }));
}
...
function renderJobs() {
  ...
  const warnings = state.lastErrors.length
    ? `<div class="warning">Some sources could not be reached: ${escapeHTML(unique(state.lastErrors).join(", "))}</div>`
    : "";
    ...
    <article class="job-card">
      <div>
        ...
        <div class="job-actions">
          <a class="apply-link" href="${escapeAttribute(job.url)}" target="_blank" rel="noreferrer">Apply / View job</a>
